...
class IngredientTest {
  ...
  void createIngredient_withInvalidValues_shouldThrowException() {
    try {
      new Ingredient(fakeIdGenerator, null, 1.0, 5.0, Unit.L);
      assert false;
    } catch (IllegalArgumentException ignored) {
    }
    try {
      new Ingredient(fakeIdGenerator, "", 1.0, 5.0, Unit.L);
      assert false;
    } catch (IllegalArgumentException ignored) {
    }

    try {
      new Ingredient(fakeIdGenerator, "Leite", 0, 5.0, Unit.L);
      assert false;
    } catch (IllegalArgumentException ignored) {
    }

    try {
      new Ingredient(fakeIdGenerator, "Leite", 1.0, -1.0, Unit.L);
      assert false;
    } catch (IllegalArgumentException ignored) {
    }

    try {
      new Ingredient(fakeIdGenerator, "Leite", 1.0, 5.0, null);
      assert false;
    } catch (IllegalArgumentException ignored) {
    }
  }
